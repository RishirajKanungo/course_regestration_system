...
<?php 
    ...
    if(!empty($search)) { 
        ...
        $searchQuery = "SELECT users.fname, users.minit, users.lname, users.UID 
                        FROM users, app
                        WHERE (app.recEmail='$email' AND app.UID=users.UID AND app.recStatus='Not Received') AND (CONCAT(fname, ' ', lname) LIKE '%".$search."%')";
...
?>
    ...
    if(!empty($search)) { 
        ...
        $searchQuery = "SELECT users.fname, users.minit, users.lname, app.appDate 
                        FROM users, app
                        WHERE (app.recEmail='$email' AND app.UID=users.UID AND app.recStatus='Received') AND (CONCAT(fname, ' ', lname) LIKE '%".$search."%')";
        ...
        $query = ("SELECT users.fname, users.minit, users.lname, history.appDate
               FROM users, history
               WHERE (history.recEmail='$email' AND history.UID=users.UID) AND (CONCAT(fname, ' ', lname) LIKE '%".$search."%')"); 
        $data = mysqli_query($dbc, $query);
        $numRowsH = mysqli_num_rows($data);
        if (($numRows == 0) && ($numRowsH == 0)){
            echo "</br><h5>No past requests for student: </h5>";
            echo $search;
        }
